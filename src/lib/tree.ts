...
export const findPathToNode = (root: ConceptNode, id: string): ConceptNode[] | null => {
  if (root.id === id) return [root]

  for (const child of root.children) {
    const subPath = findPathToNode(child, id)
...
export const safeResolveNode = (root: ConceptNode, id: string): ConceptNode => {
